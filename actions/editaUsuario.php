...
<html>
	...
	<body>
		<?php
			...
			$usuario->usuCodigo=$_POST["txtCodigo"];
			$usuario->usuTipCodigo=$_POST["txtTipo"];
			$usuario->usuNick=$_POST["txtNick"];
			$usuario->usuEmail=$_POST["txtEmail"];
			$usuario->usuPassword=$_POST["txtPassword"];
			
			$dao = new UsuarioDao();
			$inseriu = $dao->editaUsuario($usuario);
			...
			if($inseriu){
				...
				foreach ($inseriu as $usr) {
					...
					if ($_SESSION["usutipo"] == "2" ){
						"<a href='../forms/formListaUsuario.php' class='ui-block-b' data-role='button' data-theme='c'>Continuar</a>
						    </div>
						  </div>
						</div>";
					}else{
						"<a href='../index.php' class='ui-block-b' data-role='button' data-theme='c'>Continuar</a>
						    </div>
						  </div>
						</div>";
					}
