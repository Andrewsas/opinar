...
<html>
	<head>
		<title>Não Gostei Comentario</title>
		<meta charset="utf-8">
	...
	<body>
		<?php
			...
			require_once("../dao/AvaliaComentarioDAO.php");

			$usuario = new  Usuario();
			$item = new  Item();

			$usuarioatual=$_SESSION["usucodigo"];
			$usuario->codigo=$_GET["txtUsuarioCodigo"];;
			$item->codigo=$_GET["txtItemCodigo"];
			
			$dao = new AvaliaComentarioDAO();
			$inseriu = $dao->naogosteiCometario($item, $usuario, $usuarioatual);
